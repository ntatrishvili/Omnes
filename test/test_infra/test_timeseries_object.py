...
class TestTimeseriesObjectExtended(unittest.TestCase):
    ...
    def test_to_1h(self):
        ...
        np.testing.assert_array_equal(ts_1h.to_df().values.flatten(), self.df.values.flatten())

    def test_to_15m(self):
        ts_15m = self.ts.to_15m()
        self.assertEqual(ts_15m.freq, "15min")
        # With xarray backend, check the data size
        self.assertEqual(ts_15m.data.sizes['time'], (len(self.df) - 1) * 4 + 1)  # interpolated
...
class TestXArrayFeatures(unittest.TestCase):
    ...
    def test_selection_operations(self):
        ...
        # Select time range
        subset_ts = self.ts_multi.sel(time=slice('2025-01-01 06:00', '2025-01-01 12:00'))
    ...
    def test_complex_energy_scenario(self):
        ...
        self.assertGreater(df_export.shape[1], 1)  # Multiple columns for multi-dimensional data
