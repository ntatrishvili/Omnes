...
class Converter(object):
    ...
    def convert_entity(self, entity: Entity, time_set: Optional[TimeSet] = None):
        ...
        """
        log.debug(
            f"Converting entity '{entity.id}' with quantities: {list(entity.quantities.keys())}"
        )
        ...
        if hasattr(entity, "sub_entities"):
            for _, sub_entity in entity.sub_entities.items():
                log.debug(
                    f"Recursively converting '{entity.id}' sub-entity '{sub_entity.id}'"
                )
    ...
    def convert_model(self, model: Model, **kwargs):
        ...
        **kwargs
        ...
        effective_time_set, context = self._prepare_conversion(
            model, time_set=time_set, **kwargs
        )
        ...
        result = self._post_process_conversion(
            model, result, effective_time_set, context, **kwargs
        )
    ...
    def _finalize_result(
        self, result: Any, time_set: TimeSet, context: Dict[str, Any]
    ) -> Any:
        """
